    ...
    public function createSettlement(Request $request): JsonResponse
    {
        ...
        $settlement = DB::transaction(function () use ($companyId, $validated, $request): Settlement {
            ...
            foreach ($commissions as $commission) {
                $settlement->items()->create(['company_id' => $companyId, 'booking_id' => $commission->booking_id, 'commission_id' => $commission->id, 'code' => $settlement->code.':'.$commission->id, 'name' => 'Booking '.$commission->booking->reference, 'status' => 'pending', 'currency' => $commission->currency, 'gross_amount' => $commission->gross_amount, 'fee_amount' => (float) $commission->platform_amount + (float) $commission->agent_amount, 'net_amount' => $commission->operator_amount]);
            }
